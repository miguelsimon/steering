...
auto RotationRRTProblem::plan(Vector3f src, Vector3f goal)
    ...
    for (auto candidate_action : actions) {
        auto candidate_dst = src + candidate_action;
        auto candidate_distance = (candidate_dst - goal).norm();
        if (is_free(candidate_dst)) {
            if (!found) {
                found = true;
    ...
    return std::make_tuple(found, dst, action);
};
...
auto RotationRRTProblem::segments_intersect(Vector2f a, Vector2f b, Vector2f c,
                                            Vector2f d) -> bool {
...
auto RotationRRTProblem::to_segment_endpoints(Vector3f p, float length)
